...
class EqualLR:
    def __init__(self, name):
        self.name = name
    #def symbolic(self, input):
       # return g.op("equal_lr", input, self.name)
    def compute_weight(self, module):
        weight = getattr(module, self.name + '_orig')
        fan_in = weight.data.size(1) * weight.data[0][0].numel()
        weight = torch.Tensor(weight)
        #print(type(weight), type(fan_in))
...
class StyledGenerator(nn.Module):
    ...
    def forward(
        ...
        for i in range(10):
            style = self.mean_style(torch.randn(1024, 512).to('cpu'))
        ...
        if noise is None:
            ...
            for i in range(step + 1):
                size = 4 * 2 ** i
                noise.append(torch.randn(1, 1, size, size, device=input[0].device))
